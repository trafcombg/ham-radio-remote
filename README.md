...
преоткриваме) — `Discover.discover_single()` изисква имейл+парола на
TP-Link/Tapo акаунта, с който е бил настроен контактът (само за локалния
handshake, нищо не излиза в интернет).
...
- **TPAP протокол / неофициален fork** — по-новите Tapo устройства (вкл.
  тествания P100) говорят нова TP-Link схема за криптиране, TPAP, която
  официалният `python-kasa` (PyPI) все още не поддържа
  ([issue #1590](https://github.com/python-kasa/python-kasa/issues/1590),
  отворен от 2025-10, все още отворен към 2026-09). `server/requirements.txt`
  сочи към неmerge-натия `ZeliardM/python-kasa@feature/tpap` fork
  ([PR #1592](https://github.com/python-kasa/python-kasa/pull/1592)) вместо
  PyPI release — смени обратно към обикновен `python-kasa`, щом този PR
  влезе в официален release.
- Discovery/протоколът е потвърден срещу реален P100 (LZ4TL, 2026-09) —
  самото свързване вече разпознава TPAP правилно; пълен on/off тест с
  истинските Tapo креденшъли предстои от операторската страна.
  `python -m server.tapo_bridge` тества само логиката с фалшиво
  устройство, не истинска мрежова връзка.
